...
const PostCard = ({post}) => {
    ...
    const [liked, setLikes] = useState(post.likes_count);
          ...
          <div className='text-gray-800 text-sm whitespace-pre-line' dangerouslySetInnerHTML={{__html: postWithHashtags}}></div>
            ...
            <div className='grid grid-cols-2 gap-2'>
                {post.image_urls.map((url, index) => (
                  <img key={index} src={url} alt="" className='w-full h-48 object-cover rounded-lg ${post.image_urls.length === 1 && "col-span-2 h-auto"}'} alt=""/>
                ))}
          </div>
